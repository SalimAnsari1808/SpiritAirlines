...
public class Bag_Embargo_Testing_Assistance_OneWay_Dom_Int extends BaseClass {
    ...
    public void Bag_Embargo_Test(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID Bag_Embargo_Testing_Assistance_RoundTrip under GoldFinger Suite on " + platform + " Browser", true);
        }
...
//        for (int i = 0; i < 1; i++){
            ...
            final String ARR_AIRPORT_CODE 	= INTERNATIONAL_FLIGHTS.get(i);
...
//            pageMethodManager.getFlightAvailabilityMethods().selectFlightNatureType("Ret", ARR_Flight); // one way
