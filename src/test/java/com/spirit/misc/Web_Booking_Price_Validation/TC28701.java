...
//Test Case ID: TC28701
//Description :Task 28701: 9FC Flight + Hotel + Car DOM RT. Validate prices on Dynamic Shoping Cart on Passenger Information,
// Bags, Seats and Options page. Validating prices on Payment, Confirmation, Reservation Summary, Itinerary Receipt Page.
// Validate cannot be modified or cancel
//Created By  : Gabriela
//Created on  : 18-Dec-2019
//Reviewed By : Shourya Ravula
//Reviewed On : 26-Dec-2019
//*********************************************************************************************************
public class TC28701 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"ActiveBug", "BookPath", "RoundTrip", "DomesticDomestic","FlightHotelCar", "Outside21Days", "Adult",
                    "NineDFC","CarsUI", "HotelsUI", "BookIt", "NoBags", "NoSeats", "DynamicShoppingCartUI",
                    "CheckInOptions","PaymentUI","Visa","ConfirmationUI","ReservationUI","ItineraryReceiptUI"})
    public void Flight_Hotel_Car_DOM_RT_Validate_prices_on_Dynamic_Shoping_Cart_on_Passenger_Information_Bags_Seats_and_Options_page_Validating_prices_on_Payment_Confirmation_Reservation_Summary_Itinerary_Receipt_Page_Validate_cannot_be_modified_or_cancel(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC28701 under Web_Booking_Price_Validation Suite on " + platform + " Browser", true);
        }
...
//Cars Page Methods
        ...
        //TODO: [IN:25529]	GoldFinger R1: CP: BP: Car upsell on Hub Page: 0.01 cent is missing from the total car price when adding car on Options page
        ValidationUtil.validateTestStep("Validating total price information displayed is correct",
                Math.rint((carPrice * daysTotal)*100)/100 == carTotalPrice);
//                carPrice * daysTotal == carTotalPrice);
...
//Passenger Information Page Validations
        ...
        ValidationUtil.validateTestStep("Validating total price displayed match with the expected result",
                itineraryPrice == Math.rint((roomPrice + carTotalPrice)*100)/100);
...
//Payment Page Validations
        ...
        ValidationUtil.validateTestStep("Validating departure and returning prices are matching with the total due",
                depPrice + retPrice == totalDuePrice);
...
//Confirmation Page Validations
        ...
        try {
            ...
            ValidationUtil.validateTestStep("Validating departure and returning prices are matching with the total due",
                    depPricePaid + retPricePaid == totalPaid);
...
//Itinerary Receipt Page Validation
            ...
            ValidationUtil.validateTestStep("Validating departure and returning prices are matching with the total due",
                    depPricePaid1 + retPricePaid1 == totalPaid1);
