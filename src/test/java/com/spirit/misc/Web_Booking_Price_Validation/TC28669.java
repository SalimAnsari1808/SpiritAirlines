...
//Test Case ID: TC28669
//Description : Task 28669: 9FC Car Upsell DOM RT. Validate prices on Dynamic Shoping Cart on Passenger Information,
...
//*********************************************************************************************************
public class TC28669 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"ActiveBug", "BookPath", "RoundTrip", "DomesticDomestic", "Outside21Days", "Adult",
    ...
    public void CarUpsell_DOM_RT_Validate_prices_on_Dynamic_Shoping_Cart_on_Passenger_Information__Bags_Seats_and_Options_page_Validating_prices_on_Payment_Confirmation_Reservation_Summary_Itinerary_Receipt_Page_Validate_cannot_be_modified_or_cancelled_on_MT(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC28669 under Web_Booking_Price_Validation Suite on " + platform + " Browser", true);
        }
...
//Flight Availability Page Method
        ...
        {
            totalPrice1 = pageObjectManager.getFlightAvailabilityPage().getMemberFarePriceText().getText().replace("$","");
        }else
            totalPrice1 = pageObjectManager.getFlightAvailabilityPage().getFlightTotalAmountText().getText().replace("$","");
