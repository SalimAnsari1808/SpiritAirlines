...
//Test Case ID: TC28659
//Description : FS Hote Upsell DOM RT. Validate prices on Dynamic Shoping Cart on Passenger Information, Bags, Seats and
//              Options page.  Validating prices on Payment, Confirmation, Reservation Summary, Itinerary Receipt Page.
...
public class TC28659 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"ActiveBug","BookPath", "OneWay", "DomesticDomestic","WithIn7Days", "Adult", "FreeSpirit",
                    "Connecting", "BookIt", "NoBags", "NoSeats","Hotels", "HotelsUI", "CheckInOptions", "Visa",
                    "DynamicShoppingCartUI","PaymentUI","ConfirmationUI", "ReservationUI" , "CheckIn"})
    public void FS_Hote_Upsell_DOM_RT_Validate_prices_on_Dynamic_Shoping_Cart_on_Passenger_Information_Bags_Seats_and_Options_page_Validating_prices_on_Payment_Confirmation_Reservation_Summary_Itinerary_Receipt_Page_Validate_cannot_be_modified_or_cancelled_on_MT(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC28659 under Web_Booking_Price_Validation Suite on " + platform + " Browser", true);
        }
        ...
        ValidationUtil.validateTestStep("Validating Flight + Hotel itinerary is displayed on the Dynamic Shopping Cart on Options Page",
