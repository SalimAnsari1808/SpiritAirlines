...
//TODO: bug 25717: Prod: CP: CI : Boarding Pass: "Bag Drop" time and "Self Bag-Tag" time is displaying the same time as boarding time
//**********************************************************************************************
//Test Case ID: TC90930
//Description: BoardingPass_CP_CI_INT_OW_Verify you get self tag airport message departing from a Self-Tagging Airports
...
public class TC90930 extends BaseClass {

    @Parameters({"platform"})
    @Test(groups = {"ActiveBug" , "CheckIn" , "RoundTrip" , "DomesticColombia" , "WithIn7Days" , "Adult" , "FreeSpirit" , "NonStop" , "BookIt" , "CheckedBags" , "NoSeats" , "CheckInOptions" , "Visa" ,"AddEditPassportInfo", "BoardingPassUI"})
    public void BoardingPass_BoardingPass_CP_CI_INT_OW_Verify_you_get_self_tag_airport_message_departing_from_a_Self_Tagging_Airports(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC90930 under REGRESSION-CRITICAL Suite on " + platform + " Browser", true);
        }
...
//Step 3 and 4: Print Boarding pass
...
//Step 7: Validate the Self Bag Tag airport verbiage is correct
        //International self bag tag times is 90 minutes from Standard Time of departure
        String bagDropExpectedTime = getDesiredTime(pageObjectManager.getBoardingPassPage().getDepartsCityTimeText().get(0).getText().trim() , -1 , -30 , "HH:mm");

        System.out.println(pageObjectManager.getBoardingPassPage().getSelfBagTagText().get(0).getText());
