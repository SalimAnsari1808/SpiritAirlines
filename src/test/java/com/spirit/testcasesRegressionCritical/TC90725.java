...
public class TC90725 extends BaseClass {

    @Parameters({"platform"})
    @Test(groups = {"BookPath" , "OneWay" , "DomesticDomestic" , "WithIn7Days" , "Adult" , "Guest" , "Nonstop" , "BundleIt" , "CarryOn" ,"CheckedBags","BagsUI", "BigFront" ,"SeatsUI", "DynamicShoppingCartUI"})
    public void Dynamic_Shopping_Cart_Seats_page_WireFrame(@Optional("NA") String platform) {
        ...
        if(!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC90725 under REGRESSION-CRITICAL Suite on " + platform + " Browser"   , true);
        }
...
//STEP--1
 ...
 //STEP--2
        ...
        for(WebElement check:pageObjectManager.getHeader().getBreadCumCheckMarkImage()){
            ValidationUtil.validateTestStep("BreadCum check mark back ground color is blue",check.getCssValue("background-color").contains(BLUE_RGB));
            ValidationUtil.validateTestStep("BreadCum check mark color is white",check.findElement(By.tagName("i")).getCssValue("color").contains(WHITE_RGB));
        }

        ValidationUtil.validateTestStep("BreadCum Active Step Number back ground color is black",
                pageObjectManager.getHeader().getBreadCumActiveStepNumberText().getCssValue("background-color").contains(BLACK_RGB));

        ValidationUtil.validateTestStep("BreadCum Active Step Number color is white",
                pageObjectManager.getHeader().getBreadCumActiveStepNumberText().findElement(By.tagName("p")).getCssValue("color").contains(WHITE_RGB));
        ...
        ValidationUtil.validateTestStep("Verify A Grey text block",
                getDriver().findElement(By.xpath("//div[contains(@class,'total-cost')]")).getCssValue("background-color").contains(BACKGROUND_GRAY));
...
//STEP--3
        ...
        ValidationUtil.validateTestStep("verify flight price is displayed",
                TestUtil.verifyElementDisplayed(pageObjectManager.getHeader().getFlightPriceItineraryText())
                        && pageObjectManager.getHeader().getFlightPriceItineraryText().getText().trim().equals("$" + String.format("%.2f", FlIGHTTOTAL)));
        //verify arrow
...
//STEP--7
        ...
        ValidationUtil.validateTestStep("Verify $9FC tile yellow background",
                getDriver().findElement(By.xpath("//div[contains(@class,'flight-club-savings')]")).getCssValue("background-color").contains(BACKGROUND_YELLOW));
