...
public class TC91118 extends BaseClass {

    @Parameters({"platform"})
    @Test(groups = {"BookPath" , "OneWay" , "DomesticDomestic" , "WithIn7Days" , "Adult" , "NineDFC" , "NonStop" , "BookIt" , "MandatoryFields" , "PassengerInfoSignIn" ,"PassengerInformationUI", "NoBags" , "NoSeats" ,"CheckInOptions", "MasterCard" , "ConfirmationUI"})
    public void Customer_Info_CP_BP_Enter_information_for_a_9DFC_Pax_then_login(@Optional("NA") String platform) {

        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC91118 under REGRESSION-CRITICAL suite on " + platform + " Browser", true);
        }
...
//-- Step 5: Log in as a 9DFC member on the customer information page with the inline login
        ...
        ValidationUtil.validateTestStep("Validating Right Last Name displayed on Contact section",
                pageObjectManager.getPassengerInfoPage().getContactPersonLastNameTextBox().get(0).getAttribute("value"),passengerInfoData.lastName);
