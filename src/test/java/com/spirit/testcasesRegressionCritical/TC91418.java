...
//**********************************************************************************************
public class TC91418 extends BaseClass {

    @Parameters({"platform"})
    @Test(groups = {"BookPath" , "MultiCity" , "DomesticDomestic" , "WithIn7Days" , "Adult" , "Guest" , "NonStop" , "BookIt" , "CarryOn" , "CheckedBags" , "NoSeats" ,"ShortCutBoarding","CheckInOptions", "TravelInsurancePopUp" , "Visa","PaymentUI"})
    public void CP_BP_Payment_Page_Itinerary_MC_Validate_EDIT_Bags_Links(@Optional("NA") String platform) {
        ...
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC91418 under REGRESSION-CRITICAL Suite on " + platform + " Browser", true);
        }
        ...
        ValidationUtil.validateTestStep("Verify No Bags are selected for Departing Flight on Passenger Section on Payment Page",
                pageObjectManager.getPaymentPage().getAllBagsInfoText().get(0).getText(),NO_BAGS);

        ValidationUtil.validateTestStep("Verify No Bags are selected for Arrival Flight on Passenger Section on Payment Page",
