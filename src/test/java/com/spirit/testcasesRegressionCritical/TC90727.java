...
public class TC90727 extends BaseClass{

    @Parameters({"platform"})
    @Test(groups = {"BookPath" , "OneWay" , "DomesticDomestic" , "Outside21Days" , "Adult" , "Guest" , "NewFlightSearch" ,"Connecting,", "BookIt" ,"PassengerInformationUI" , "DynamicShoppingCartUI"})
    public void Dynamic_Shopping_Cart_Flights_Caret_Dropdown_WireFrame(@Optional("NA") String platform) {
        ...
        if(!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC90727 under REGRESSION-CRITICAL Suite on " + platform + " Browser" , true);
        }
        ...
        ValidationUtil.validateTestStep("verify Edit link(blue font) is displayed",
                TestUtil.verifyElementDisplayed(pageObjectManager.getHeader().getEditFlightItineraryButton())
                        && pageObjectManager.getHeader().getEditFlightItineraryButton().getCssValue("color").contains(BLUE_RGB));
