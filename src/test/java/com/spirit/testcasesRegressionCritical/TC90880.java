...
//**********************************************************************************************
//Test Case ID: TC90880
//Description: Task 24654: 35419 BoardingPass_CP_CI_Email delivery option check both boxes
...
public class TC90880 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"CheckIn" , "OneWay" , "DomesticDomestic" , "WithIn7Days" , "Adult" , "FreeSpirit" , "NonStop" , "BookIt" , "NoBags" , "NoSeats" , "CheckInOptions" , "Visa" , "BoardingPassUI" ,"ReservationUI", "Email"})
    public void BoardingPass_CP_CI_Email_delivery_option_check_both_boxes(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC90880 under REGRESSION-CRITICAL Suite on " + platform + " Browser", true);
        }
...
//Step:3 Click close on the BoardingPass Modal, boarding pass should not display
        pageObjectManager.getReservationSummaryPage().getYourBoardingPassPopUpCloseImage().click();

//Step 4: proceed to print option
        pageObjectManager.getOptionsPage().getContinueToPurchaseButton().click();//click continue on the extras page

        pageMethodManager.getReservationSummaryPageMethods().acceptRejectHazardousMaterialPopup(HAZMET_POPUP);//Accept Hazmat
        //wait till page is load is complete
        WaitUtil.untilPageLoadComplete(getDriver());
        WaitUtil.untilTimeCompleted(2000);

//Step 5: Select both options
        ...
        ValidationUtil.validateTestStep("The Pre-populated email is correct",
                pageObjectManager.getReservationSummaryPage().getYourBoardingPassPopupEmailBoardingPassTextBox().getAttribute("value"),
                FileReaderManager.getInstance().getJsonReader().getCredentialsByUserType("FSEmail").userName );

        TestUtil.clearTextBoxUsingSendKeys(getDriver(),pageObjectManager.getReservationSummaryPage().getYourBoardingPassPopupEmailBoardingPassTextBox());//clear default value
        pageObjectManager.getReservationSummaryPage().getYourBoardingPassPopupEmailBoardingPassTextBox().sendKeys("emailtesters@spirit.com");//input emailtesters inbox

//Step 6: Click on Finish check in button

        WaitUtil.untilPageLoadComplete(getDriver());
        ((JavascriptExecutor)getDriver()).executeScript("window.print=function(){};");//close
        WaitUtil.untilTimeCompleted(2000);
...
//Step 7: Validate boarding pass is in inbox
        //get current time
...
//Step 9: validate the lower left quadrant
        ...
        ValidationUtil.validateTestStep("Name on top left quadrant is correct", pageObjectManager.getBoardingPassPage().getUserNameLeftBottomText().get(0).getText().contains(passengerInfoData.firstName.toUpperCase())
        ...
        ValidationUtil.validateTestStep("The Bar Code is displayed",
                TestUtil.verifyElementDisplayed(pageObjectManager.getBoardingPassPage().getBoardingPassImage().get(0)));
...
//Step--10
        ...
        ValidationUtil.validateTestStep("'BOARDING TIME' verbiage is displaying correct",
                TestUtil.verifyElementDisplayed(pageObjectManager.getBoardingPassPage().getBoardingActualTimeText().get(0)));
        ValidationUtil.validateTestStep("'BOARDING TIME' verbiage is displaying correct",
        ...
        ValidationUtil.validateTestStep("'SEAT' verbiage is displaying correct",
                TestUtil.verifyElementDisplayed(pageObjectManager.getBoardingPassPage().getSeatNumberText().get(0)));
        ...
        ValidationUtil.validateTestStep("Depart Time is displaying correct",
                pageObjectManager.getBoardingPassPage().getArriveCityTimeText().get(0).isDisplayed());
        ...
        ValidationUtil.validateTestStep("Depart Time is displaying correct",
                pageObjectManager.getBoardingPassPage().getDepartsCityNameText().get(0).isDisplayed());
        ...
        ValidationUtil.validateTestStep("Issued date is correct",
                pageObjectManager.getBoardingPassPage().getIssuedTimeText().get(0).getText().trim().contains(newformat1.format(date)));
