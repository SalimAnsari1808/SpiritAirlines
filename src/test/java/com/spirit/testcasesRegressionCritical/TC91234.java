...
// **********************************************************************************************
// TestCase   : My Account_CP_$9FC_ Member Statement Link
...
public class TC91234 extends BaseClass {

    @Parameters({"platform"})
    @Test(groups = {"NineDFC" , "AccountProfileUI"})
    public void My_Account_CP_$9FC_Member_Statement_Link(@Optional("NA") String platform) {

        //Mention Suite and Browser in reports
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC91234 under REGRESSION-CRITICAL Suite on " + platform + " Browser", true);
        }
...
//Step 7: Validate the statements header
        ...
        ValidationUtil.validateTestStep("The statements header is displayed correctly" , pageObjectManager.getAccountProfilePage().getStatementsHeaderText().getText() , "Statements");
...
///Step 9: Validate the transaction types on the drop down
        ...
        for (int count = 0 ; count < transactionOptionsACTUAL.size() ; count++ ) {
            ValidationUtil.validateTestStep("The transaction type drop down is displayed for " + transactionOptionsEXPECTED.get(count) ,
...
//Step 12 and 13: VALIDATE THAT THE DATES ARE IN mm/dd/yyyy format
        ...
        for (WebElement dateElement : pageObjectManager.getAccountProfilePage().getStatementTableDatesText()) {
            String dateListed = dateElement.getText().substring(0,10) + " " + dateElement.getText().substring(11,dateElement.getText().length());

            Date date = TestUtil.convertStringToDate(dateListed,"MM/dd/YYYY HH:MM a");
...
//Step 16: Validate bottom of page text "*Beginning Balance as of MM/1/YY"
        ...
        ValidationUtil.validateTestStep("The beginning balance text is correct" , pageObjectManager.getAccountProfilePage().getStatementBeginningBalanceDate().getText() ,
