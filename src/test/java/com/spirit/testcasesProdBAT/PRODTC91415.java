...
public class PRODTC91415 extends BaseClass {
    ...
    public void CP_BP_Payment_Page_Itinerary_RT_Validate_EDIT_Flight_link_on_payment_page(@Optional("NA") String platform) {
        ...
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID PRODTC91415 under PRODUCTION Suite on " + platform + " Browser", true);
        }
...
//-- Step 8:
        ...
        WaitUtil.untilTimeCompleted(1000);
        for (int count = 0; count < pageObjectManager.getFlightAvailabilityPage().getArrivalFlightEditButton().size(); count++) {
            if (pageObjectManager.getFlightAvailabilityPage().getArrivalFlightEditButton().get(count).isDisplayed()) {
                JSExecuteUtil.clickOnElement(getDriver(), pageObjectManager.getFlightAvailabilityPage().getArrivalFlightEditButton().get(count));
            }
        }
