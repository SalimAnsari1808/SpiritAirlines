...
public class TC349829 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"BookPath", "RoundTrip", "DomesticDomestic", "FlightHotelCar", "Outside21Days", "Adult", "Guest"})
    public void CP_Flight_Hotel_Availability_Page_NEG_Validating_One_ADT_is_required_per_room(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC349829 under GoldFinger Suite on " + platform + " Browser", true);
        }
...
//Step 3: Under the "Choose Your Hotel" section, validate there is a vertical section left aligned with filters
...
//- Step 4: Validate the first filter is "Hotel Search"
...
//- Step 9: Drag the start (left)  handle and the end (right) handles (circles) to adjust the price range
        //Moving slider to lef for higher price
...
//- Step 11: : Validate a "x Clear All Filters" hyperlink displays above the filters.
        ...
        TestUtil.mouseClickOnElement(getDriver() , pageObjectManager.getCarPage().getCarsPageClearAllFiltersButton());


//- Step 12: Select a Hotel and continue to the next page
        WaitUtil.untilPageLoadComplete(getDriver());
        pageMethodManager.getHotelPageMethods().selectHotelRoomHotelPage("MGM","NA");

//- Step 13: Locate the Filters section on the left side of the page
        // Price
...
//- Step 14: Validate each filter contains a pointing down arrow that when clicked it expands and the arrow turns up
        //Price
...
//- Step 15: Click the arrows for each filter and validate check boxes are displayed when the section is expanded.
        //Price
        try {
...
//- Step 16: Validate each check box displays an inventory count
...
//- Step 17: Use other filters like Rental Agency and the price slider to narrow the search. The objective is to have non qualifying check boxes gray out.
        //Invalid Step

//- Step 18: Click on and off all check boxes and validate they show selected or unselected
...
//- Step 19: Validate a "Clear" filter displays once a check box selection is made
...
//- Step 20: Apply filters to narrow down the search until no results display
...
//- Step 21: Click the "Clear All" filter to remove all filters
        pageObjectManager.getCarPage().getCarsPageClearAllFiltersButton().click();
        WaitUtil.untilPageLoadComplete(getDriver());

//- Step 22: Now go to the Price Slider, leave the left (min) circle where it is, take the right (max) and move it to the left to approximately 1/3
        ...
        //Saving higher price in a variable
        double higherPrice = Double.parseDouble(carPriceRange.get(1).getText().replace("$",""));
        ...
        ValidationUtil.validateTestStep("Validating price decrease",
                higherPrice > Double.parseDouble(carPriceRange.get(1).getText().replace("$","") ));
...
//- Step 23: Now take the  left (min) circle and move it to the middle, then take the right (max) and move it to the left to approximately half the slider, leaving a space between left and right circle
...
//- Step 24: Now leave the  left (min) circle where it is and take the right (max) and move it all the way to the right.
        //Moving the right circle to the left for decrease the price
