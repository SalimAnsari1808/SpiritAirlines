...
//**********************************************************************************************
//Test Case ID: TC373736
//Description: Email Confirmation - Car Upsell - Validate the proper email is sent for a Standard booking
//Created By: Salim Ansari
//Created On: 02-Dec-2019
//Reviewed By:
//Reviewed On:
//**********************************************************************************************
public class TC373736 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"BookPath", "RoundTrip", "FlightCar", "DomesticDomestic", "Outside21Days", "Adult", "Guest", "Connecting", "BookIt","ContactInformation",
            "NoBags","NoSeats","CheckInOptions","OptionalUI","Visa","PaymentUI","ConfirmationUI","Email"})
    public void Email_Confirmation_Car_Upsell_Validate_the_proper_email_is_sent_for_a_Standard_booking(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC373736 under GoldFinger Suite on " + platform + " Browser", true);
        }
...
//- Step 2: Create a regular booking for 1 Adt age 21 or over year old, DOM ( to any city diferent than MCO, CUN, FLL, DEN), RT, outside 48 hrs.
...
//- Step 15: On the confirmation page verify that a PNR and the HBG Code are created for the booking
        ...
        System.out.println(pageObjectManager.getConfirmationPage().getBookingSectionTopConfirmationCode().get(0).getText());
...
//- Step 17: Validate that the new packaging vendors, called carnect, infomation is on the Email
//- Step 18: Validate price shown on email is a packaging price (flightl+car as one price)
        try {
