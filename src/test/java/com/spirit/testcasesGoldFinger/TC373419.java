...
//**********************************************************************************************
//Test Case ID: TC373419
//Description: Task 27782: TC373419- 006 - CP - Car Tax Verbiage (with Hotel) - Flight + Hotel + Car - Validate the verbiage for a booking on a Thru Flight
//Created By: Gabriela
...
public class TC373419 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"BookPath", "RoundTrip", "DomesticDomestic", "FlightHotelCar", "Outside21Days", "Adult", "Guest","Through", "FlightAvailabilityUI", "CarsUI", "HotelsUI"})
    public void CP_Car_Tax_Verbiage_with_Hotel_Flight_Hotel_Car_Validate_the_verbiage_for_a_booking_on_a_Thru_Flight(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC373419 under GoldFinger Suite on " + platform + " Browser", true);
        }
...
//- Step 3: Start a Vacation Flight + Hotel + Car booking | DOM to DOM| outside 48h | 5 ADT | Driver's age 25 +|  2 Rooms | Select "Search Vacation"
        ...
        for (int i =0; i < pageObjectManager.getFlightAvailabilityPage().getDepartingNumebrOfStopsButton().size(); i ++)
        {
            if (pageObjectManager.getFlightAvailabilityPage().getDepartingNumebrOfStopsButton().get(i).getText().equals("1 Stop"))
            {
                ...
                if (pageObjectManager.getFlightAvailabilityPage().getStopsPopUpFlightsNumberText().get(0).getText().equals(pageObjectManager.getFlightAvailabilityPage().getStopsPopUpFlightsNumberText().get(1).getText()))
                {
                    ...
                    pageObjectManager.getFlightAvailabilityPage().getDepartingStandardFarePriceText().get(i-1).click();
                    break;
                }
                pageObjectManager.getFlightAvailabilityPage().getStopsPopUpCloseButton().click();
                WaitUtil.untilTimeCompleted(1200);
        ...
        for (int i =0; i < retStops.size(); i ++)
        {
            if (retStops.get(i).getText().equals("1 Stop"))
            {
                ...
                if (pageObjectManager.getFlightAvailabilityPage().getStopsPopUpFlightsNumberText().get(0).getText().equals(pageObjectManager.getFlightAvailabilityPage().getStopsPopUpFlightsNumberText().get(1).getText()))
                {
                    ...
                    pageObjectManager.getFlightAvailabilityPage().getReturningStandardFarePriceText().get(i-1).click();
                    break;
                }
                pageObjectManager.getFlightAvailabilityPage().getStopsPopUpCloseButton().click();
                WaitUtil.untilTimeCompleted(1200);
...
//- Step 10: On the Car availability page scroll down to a thrifty car content box
        pageMethodManager.getCarPageMethods().filterCarByRentalAgency("Thrifty");

//- Step 11: Click "More" in the tile of one of the Thrifty cars
