...
//**********************************************************************************************
//Test Case ID: TC373420
//Description: Task 27869: TC373420 - 007 - CP - Car Tax Verbiage (with Hotel) - Flight + Hotel + Car - Validate the verbiage for a booking on a Connecting Flight
// Created By: Gabriela
//Created On: 30-Nov-2019
//Reviewed By:
//Reviewed On:
//**********************************************************************************************
public class TC373420 extends BaseClass {
    @Parameters({"platform"})
    @Test(groups = {"BookPath", "RoundTrip", "DomesticDomestic", "FlightHotelCar", "Outside21Days", "Adult", "Guest", "Connecting", "FlightAvailabilityUI", "CarsUI", "HotelsUI"})
    public void Car_Tax_Verbiage_with_Hotel_Flight_Hotel_Car_Validate_the_verbiage_for_a_booking_on_a_Connecting_Flight(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC373420 under GoldFinger Suite on " + platform + " Browser", true);
        }
...
//- Step 3: Start a Vacation Flight + Hotel + Car booking | DOM to DOM| outside 48h | 5 ADT | Driver's age 25 +|  3 Rooms | Select "Search Vacation"
...
//- Step 4: On the section Your Selected Departure check is the preselected Flight is a connecting Flight, if not click the Edit button for select a connecting flight.
        ...
        for (int i =0; i < pageObjectManager.getFlightAvailabilityPage().getDepartingNumebrOfStopsButton().size(); i ++)
        {
            if (!pageObjectManager.getFlightAvailabilityPage().getDepartingNumebrOfStopsButton().get(i).getText().equals("Nonstop"))
            {
                ...
                if (!pageObjectManager.getFlightAvailabilityPage().getStopsPopUpFlightsNumberText().get(0).getText().equals(pageObjectManager.getFlightAvailabilityPage().getStopsPopUpFlightsNumberText().get(1).getText()))
                {
                    ...
                    departingFlightList.get(i).click();
                    break;
                }
                //Close pop up
                pageObjectManager.getFlightAvailabilityPage().getStopsPopUpCloseButton().click();
...
//- Step 5: On the section Your Selected Return check is the preselected Flight is a connecting Flight, if not click the Connecting button for select a connecting flight.
...
//- Step 6: On the F + H  availability page, in the verbiage under the header Choose your Hotel locate and select the hyperlink "taxes and fees"
        JSExecuteUtil.clickOnElement(getDriver(),pageObjectManager.getFlightAvailabilityPage().getFlightAvailabilityTaxesAndFeesLink());

//- Step 7: Validate that the last sentence under the header PACKAGES/CAR RENTALS in the pop up: "Car rental rates do not include taxes, airport fees, insurance, options, underage driver charges... "
...
//- Step 11: Click "More" in the tile of one of the Dollar cars
        ...
        {
            JSExecuteUtil.clickOnElement(getDriver(),pageObjectManager.getCarPage().getCarsPageMoreInfoLink().get(i));
            WaitUtil.untilPageLoadComplete(getDriver());
