...
//**********************************************************************************************
//Test Case ID: TC373271
//Description: Task 27210: TC373271 - 003 - CP - Hotel Widget Dynamic Selection - Validate the default Room Selection for 5 passengers
//Created By: Gabriela
...
public class TC373271 extends BaseClass {
    @Parameters({"platform"})
    ...
    public void CP_Hotel_Widget_Dynamic_Selection_Validate_the_default_Room_Selection_for_5_passengers(@Optional("NA") String platform) {
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC373271 under GoldFinger Suite on " + platform + " Browser", true);
        }
...
//- Step 3: Book a Dom-Dom | outside of 3 months | Select 5 ADT
...
//- Step 4: Validate the room tab allows up to 4 rooms
        ValidationUtil.validateTestStep("Validating the room field defaults to 2 room.",
                pageObjectManager.getHomePage().getRoomsDropDown().getAttribute("value").equals("2"));

        ValidationUtil.validateTestStep("Validating there is 4 rooms option for 5 passengers available",
...
//- Step 7: Click on Continue without car
        WaitUtil.untilPageLoadComplete(getDriver());
        pageObjectManager.getCarPage().getCarsPageContinueWithoutCarButton().click();

//- Step 8: Click on book it on Upgrade & Save pop up
        WaitUtil.untilPageLoadComplete(getDriver());
        pageMethodManager.getFlightAvailabilityMethods().selectJourneyUpgrade(UPGRADE_VALUE);

//- Step 9: Fill out passenger info and continue
        pageMethodManager.getPassengerInfoMethods().fillPassengersMandatoryFields();
        pageMethodManager.getPassengerInfoMethods().fillContactMandatoryFields();
        pageMethodManager.getPassengerInfoMethods().clickContinueButton();

//- Step 10: Click on continue without bags
        pageMethodManager.getBagsPageMethods().continueWithoutSelectingBags();

//- Step 11: Click on Continue without seats
        pageMethodManager.getSeatsPageMethods().continueWithoutSeats();

//- Step 12: Select check-in free option and continue
        /***Options Page Methods**/
        pageMethodManager.getOptionsPageMethods().selectOptions(OPTIONS_VALUE);
        pageMethodManager.getOptionsPageMethods().continueToPurchaseButton();

//- Step 13: Fill out proper payment information and complete booking
