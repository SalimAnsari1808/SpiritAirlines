...
// **********************************************************************************************
//Test Case ID: TC91351
// Description: Taxes and Fees_CP_BP_RT_To Haiti (Cap Haitien)
// Created By : Kartik Chauhan
// Created On : 03-Apr-2019
// Reviewed By: Salim Ansari
// Reviewed On: 04-Apr-2019
// **********************************************************************************************
public class TC91351 extends BaseClass {

    @Parameters ({"platform"})
    @Test (groups = {"BookPath" , "RoundTrip" , "DomesticInternational" , "WithIn7Days" , "Adult" , "Guest" , "Connecting" , "BookIt" ,
            "NoBags" , "NoSeats" , "CheckInOptions" , "TravelInsuranceBlock" , "MasterCard" , "TaxesAndFee" , "PaymentUI", "ConfirmationUI"})
    public void Taxes_and_Fees_CP_BP_RT_To_Haiti_Cap_Haitien (@Optional("NA")String platform){
        //***********************Navigation to Purchase Page*****************************/
        //Mention Suite and Browser in reports
        if(!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC91351 under SMOKE Suite on " + platform + " Browser"   , true);
        }

        ValidationUtil.skipTestCase("Cap Haitien City pair is removed from Web Application",true);
        ...
        final String ARR_AIRPORT_CODE 	= "CAP";
...
//STEP--4
        ...
        taxList.add("HT-Passenger Facility Charge");
        taxList.add("HT Immigration Processing");
        ...
        for (int taxCounter = 0; taxCounter < taxList.size(); taxCounter++) {
            ...
            ValidationUtil.validateTestStep("Verifying Tax verbiages To Haiti (Cap Haitien) City pair of Tax " + (taxList.get(taxCounter)) + " on Payment Page", taxFlag);
        ...
        ValidationUtil.validateTestStep("All Taxes To Haiti (Cap Haitien) is displaying correctly for Roundtrip on Payment Page", true);
...
//STEP--6
        ...
        for (int taxCounter = 0; taxCounter < taxList.size(); taxCounter++) {
            ...
            //validate for all the taxes in console
            ValidationUtil.validateTestStep("Verifing Tax verbiages To Haiti (Cap Haitien) City pair of Tax " + (taxList.get(taxCounter)) + " on Confirmation Page", taxFlag);
        ...
        //Validate All Taxes are displaying correctly
        ValidationUtil.validateTestStep("All Taxes To Haiti (Cap Haitien) is displaying correctly for Roundtrip on Confirmation Page", true);
