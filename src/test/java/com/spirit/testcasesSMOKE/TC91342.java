...
//**********************************************************************************************
// Test Case ID: TC91342
// Description:  Taxes and Fees_CP_BP_RT_From El Salvador (San Salvador)
...
public class TC91342 extends BaseClass {
    //***********************Navigation to Purchase Page****************************/
    @Parameters ({"platform"})
    @Test (groups = {"BookPath" , "RoundTrip" , "InternationalDomestic" , "Outside21Days" , "Adult" , "Guest" , "NonStop " , "BookIt" ,
            "CarryOn" , "CheckedBags","NoSeats", "CheckInOptions" , "TravelInsuranceBlock" , "MasterCard" , "TaxesAndFee","BagsUI","PaymentUI","ConfirmationUI"})
    public void Taxes_and_Fees_CP_BP_RT_From_El_Salvador_San_Salvador (@Optional("NA")String platform){
        //Mention Suite and Browser in reports
        if(!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC91342 under SMOKE Suite on " + platform + " Browser"   , true);
        }
        ...
        final String DEP_AIRPORT_CODE 	= "SAL";
...
//STEP--3
        ...
        taxList.add("SV Airport Departure Tax");
        taxList.add("SV Immigration Fee");
        taxList.add("Tourism Contribution Fee");
        ...
        taxList.add("Value Added Tax Origin SV");
        ...
        ValidationUtil.validateTestStep("Verify VAT Taxes are displaying in Total Due Breakdown on Payment Page",
                TestUtil.verifyElementDisplayed(pageObjectManager.getPaymentPage().getDepTotalDueBagsVatPriceText()));

        //validate Vat Taxes are displaying correctly on Payment page
        ValidationUtil.validateTestStep("VAT Tax are correctly displaying on Payment Page ",
                scenarioContext.getContext(Context.BAGS_DEP_VAT).equals(Double.parseDouble(pageObjectManager.getPaymentPage().getDepTotalDueBagsVatPriceText().getText().replace("$", "").trim().toString())));
...
//STEP--4
        ...
        ValidationUtil.validateTestStep("Verify VAT Taxes are displaying in Total Due Paid on Confirmation Page",
                TestUtil.verifyElementDisplayed(pageObjectManager.getConfirmationPage().getVatTaxAmountText()));

        //Validate Vat Tax are displaying correctly on Confirmation Page
        ValidationUtil.validateTestStep("Vat Tax are correctly displaying on Confirmation Page ",
                scenarioContext.getContext(Context.BAGS_DEP_VAT).equals(Double.parseDouble(pageObjectManager.getConfirmationPage().getVatTaxAmountText().getText().replace("$", "").trim().toString())));
