...
//**********************************************************************************************
//Test CaseID: TC90953
//Title      : Check In_CP_ CI_ Validate Temporary Stay Information is displayed
// Created By : Kartik Chauhan
//Created On : 05-May-2019
//Reviewed By: Salim Ansari
//Reviewed On: 07-May-2019
//**********************************************************************************************
public class TC90953 extends BaseClass{

    @Parameters ({"platform"})
    @Test (groups = {"CheckIn" , "RoundTrip" , "ColombiaDomestic" , "WithIn7Days" , "Adult" , "Guest" , "Connecting" , "BookIt" ,
            "NoBags" , "NoSeats" , "CheckInOptions" , "PassportUI" , "MasterCard" , "AddEditPassportInfo"})
    public void Check_In_CP_CI_Validate_Temporary_Stay_Information_is_displayed(@Optional("NA")String platform) {
        /******************************************************************************
         ****************************Navigate to Bags Page*****************************
         ******************************************************************************/
        //Mention Suite and Browser in reports
        if(!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC90953 under SMOKE Suite on " + platform + " Browser"   , true);
        }
        ...
        final String RED_COLOR 		    = "220, 0, 0";
...
//Step--1
        ...
        /***Non-Us Residents will not receive ROKT POP**/
...
//STEP--6
        ...
        //change Issue of Country Drop down to non-us resident
        JSExecuteUtil.clickOnElement(getDriver(), pageObjectManager.getPassportPage().getPassengerIssuingCountryDropDown().get(0));
        TestUtil.selectDropDownUsingVisibleText(pageObjectManager.getPassportPage().getPassengerIssuingCountryDropDown().get(0),"Peru, Republic of");


        //put wait for 2 sec
        WaitUtil.untilTimeCompleted(5000);
//Step--7
        //verify Temporary Stay panel is displaying on Passport Page
        ValidationUtil.validateTestStep("Temporary Stay panel is displaying on Passport Page",pageObjectManager.getPassportPage().getTemporaryStayPanel().isDisplayed());
...
//Step--8
        JSExecuteUtil.scrollDownToElementVisible(getDriver(),pageObjectManager.getPassportPage().getTemporaryStayAddressTextBox());
        //enter addres to stay in US
        pageObjectManager.getPassportPage().getTemporaryStayAddressTextBox().sendKeys(memberEnrollmentData.address1);
//Step--9
        //enter City to stay in US
        pageObjectManager.getPassportPage().getTemporaryStayCityTextBox().sendKeys(memberEnrollmentData.city);
//Step--10
        //enter State to stay in US
        TestUtil.selectDropDownUsingVisibleText(pageObjectManager.getPassportPage().getTemporaryStayStateDropDown(),memberEnrollmentData.state);

//Step--11
        //enter Zip Code to stay in US
        pageObjectManager.getPassportPage().getTemporaryStayZipCodeTextBox().sendKeys(memberEnrollmentData.zipCode);
//Step--12
        //change country of resident to non-us resident
        pageObjectManager.getPassportPage().getPassengerCountryOfResidenceDropDown().get(0).sendKeys("Peru");
...
//STEP--14
        ...
        pageObjectManager.getPassportPage().getUhOkButton().click();
