...
//**********************************************************************************************
public class TC90961 extends BaseClass {

    @Parameters({"platform"})
    @Test(groups = {"BookPath", "Footer" , "MemberEnrollmentUI","NineDFC"})
    public void NineFC_Enrollment_from_$9FC_Enrollment_Page_Links(@Optional("NA") String platform) {

        //Mention Suite and Browser in reports
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC90961 under SMOKE Suite on " + platform + " Browser", true);
        }
        ...
        //STEP--3
        //Click the ! tooltip next to verbiage Get discounted fares and cheaper bags. An image will populate
        ...
        ValidationUtil.validateTestStep("User Validate image populates on Fare Club Enrollment Page",
        ...
        ValidationUtil.validateTestStep("User verify Get discounted fares and cheaper bags pop over verbiage is present on Fare Club Enrollment Page",
        ...
        //close Get discounted fares and cheaper bags image
        pageObjectManager.getMemberEnrollmentPage().getDiscountedFaresAndCheaperBagsPopOverButton().click();
        ...
        //Validate image populates
        ValidationUtil.validateTestStep("Validate image populates  on Fare Club Enrollment Page",
                TestUtil.verifyElementDisplayed(getDriver(),By.xpath(pageObjectManager.getMemberEnrollmentPage().xpath_FareClubPopOverBodyPlaceHolder)));

        System.out.println(pageObjectManager.getMemberEnrollmentPage().getFareClubPopOverTopText().getText());
        System.out.println(pageObjectManager.getMemberEnrollmentPage().getFareClubPopOverBottomText().getText());
        //STEP--6
        //Once image populates, Validate all verbiage is present"
        ValidationUtil.validateTestStep("User verify Covers everyone on your booking pop over verbiage is present on Fare Club Enrollment Page",
        ...
        //verify Frequently Ask Questions link
        ...
        ValidationUtil.validateTestStep("User verify popup is displayed  on Fare Club Enrollment Page", 1 == LogIn_PopUp.size());

        ValidationUtil.validateTestStep("User verify popup Header Verbiage  on Fare Club Enrollment Page", LogIn_PopUp.get(0).getText(),PopUpLogInVerbiage);
        ...
        ValidationUtil.validateTestStep("User verify no popup is displayed  on Fare Club Enrollment Page", 0 == NewLogIn_PopUp.size());
