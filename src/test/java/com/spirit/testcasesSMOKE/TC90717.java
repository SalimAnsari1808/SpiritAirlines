...
public class TC90717 extends BaseClass {

    @Parameters({"platform"})
    @Test (groups = {"BookPath" , "OneWay" , "DomesticDomestic" , "Outside21Days" , "Adult" , "Guest" , "NonStop" ,
                     "BookIt" , "BagsUI" , "DynamicShoppingCartUI"})
    public void CONTINUE_WITHOUT_BAGS_BUTTON_WITH_BAGS(@Optional("NA") String platform) {
        ...
        if (!platform.equals("NA")) {
            ValidationUtil.validateTestStep("Starting Test Case ID TC90717 under SMOKE Suite on " + platform + " Browser", true);
        }
...
//click on I don't need bags and validate that the user is taken to the Seats page and shopping cart does not contain bags
    ...
    private void validateUserTakenToSeatspageAndNoBagsInShoppingCart()
    {
        ...
        WaitUtil.untilPageLoadComplete(getDriver());
        ValidationUtil.validateTestStep("After clicking on I dont't need bags the user is not redirected to another page" ,
                getDriver().getCurrentUrl(),SEAT_PAGE_URL);
        ...
        {
            ValidationUtil.validateTestStep("The Shopping Cart does not have bags after clicking continue without bags",
                    true); //pass if exception was thrown when searching for bags on breakdown
        }
    }
}